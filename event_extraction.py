...
from language_model.transformers.modeling_electra import ElectraModel
...
class MyTextClassify:
    ...
    def get_label(self, text):
        with torch.no_grad():
            ...
            return self.MyTextClassifyModel.get_label(text_embedding[0])
...
class MySentNer:
    ...
    def get_bio(self, sent):
        with torch.no_grad():
            ...
            return self.MyNerModel.get_bio(sent_embedding[0], tokened_sent)
...
if __name__ == "__main__":
    ...
    text_classify_embedding.from_pretrained(os.path.join(args.text_classify_model_save_dir, 'embedding.bin'),
                                            config=text_classify_config)
    ...
    sent_ner_embedding.from_pretrained(os.path.join(args.sent_ner_model_save_dir, 'embedding.bin'),
                                       config=sent_ner_config)
